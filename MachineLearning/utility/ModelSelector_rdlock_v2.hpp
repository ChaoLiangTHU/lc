...
public:
	...
	LC::shared_timed_mutex model1mutex, model2mutex;  //更新到C++14后可以换为shared_timed_mutex
//	string model1version, model2version;

	std::string modelVersion; // 当前已经加载的模型所在的路径
...
//		}
	...
	LC::shared_timed_mutex& getMutex() {
		if (pCurModel == &model2)
			return model2mutex;
		else
			return model1mutex;
	...
	LC::shared_timed_mutex& getMutex(ModelClass* pModel) {
		if (pModel == &model2)
			return model2mutex;
		else
			return model1mutex;
	...
	inline std::string get_model_state() {
		std::stringstream ss;
		if (pCurModel == NULL) {
			ss << "cur model: empty; ";
...
#ifdef LCDebug
		lclogfl("start try_to_releaseUnusedModel");
...
//			std::cerr << "no models in folder: " << modelParentFolder << std::endl;
			...
			lclogpos();
			throw std::invalid_argument(std::string("no model dir in "+modelParentFolder));
		}
